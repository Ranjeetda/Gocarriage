...
class _CustomerBottomNavigationBar extends State<CustomerBottomNavigationBar> {
  ...
  final List<Widget> _screens = [CustomerHomeScreen(), HistoryScreen()];
  ...
  void initVersionCheck() async {
    final buildNo = await Utils.getBuildNumber();

    final provider = Provider.of<VersionControlProvider>(
      context,
      listen: false,
    );
  ...
  void handleVersionCheck(VersionControlProvider provider) {
    ...
    String updateType = data['updateType'];

    if (updateType == "force" || updateType == "optional") {
      showUpdateDialog(
        context,
        title: data['title'],
        message: data['message'],
        isForce: updateType == "force",
